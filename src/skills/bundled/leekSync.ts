...
const LEEK_SYNC_PROMPT = `# LeekWars AI Sync
...
3. Report differences:
   - Files only local (not on platform)
   - AIs only remote (not saved locally)
   - Files with differences (show diff)
   - Files in sync

## Important

- Always confirm with the user before overwriting code in either direction
- Create backups of remote code before pushing local changes
- Use the AI name (not ID) for filename matching
`

export function registerLeekSyncSkill(): void {
